...
class VectorStore:
    ...
    def add(self, entry_id: str, vector: list[float], metadata: dict | None = None) -> None:
        self.vectors[entry_id] = VectorEntry(id=entry_id, vector=vector, metadata=metadata or {})
    ...
    def remove_memory(self, memory_id: str) -> None:
        for entry_id in [
            key for key in self.vectors if key == memory_id or key.startswith(f"{memory_id}__")
        ]:
    ...
    def search(
        ...
        scored = [
            (entry, self._cosine(query_vector, entry.vector)) for entry in self.vectors.values()
        ]
    ...
    def hybrid_search(
        ...
        for entry_id, score in ranked:
            found = self.vectors.get(entry_id)
            if found:
