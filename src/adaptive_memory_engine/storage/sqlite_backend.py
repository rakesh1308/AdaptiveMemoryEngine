...
def _serialized(method):
    """Serialize access to the single shared SQLite connection."""
...
class SQLiteBackend:
    ...
    def save_embedding(self, memory_id: str, embedding: list[float]) -> None:
        ...
        self.conn.execute(
            "INSERT OR REPLACE INTO embeddings (memory_id, embedding, updated_at) VALUES (?, ?, ?)",
    ...
    def get_all_embeddings(self) -> OrderedDict[str, list[float]]:
        rows = self.conn.execute("SELECT memory_id, embedding FROM embeddings").fetchall()
    ...
    def get_stats(self) -> dict[str, int]:
        ...
        with_emb = self.conn.execute("SELECT COUNT(*) AS n FROM embeddings").fetchone()["n"]
    ...
    def get_versions(self, memory_id: str, limit: int = 50) -> list[dict[str, Any]]:
        rows = self.conn.execute(
            "SELECT * FROM memory_versions WHERE memory_id = ? ORDER BY created_at DESC LIMIT ?",
            (memory_id, int(limit)),
        ).fetchall()
        out = []
        for r in rows:
