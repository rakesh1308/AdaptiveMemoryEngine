...
class ChunkingStrategies:
    ...
    @staticmethod
    def fixed(
        content: str, chunk_size: int = DEFAULT_CHUNK_SIZE, overlap: int = DEFAULT_OVERLAP
    ) -> list[dict]:
        chunks: list[dict] = []
        start = 0
        idx = 0
    ...
    def semantic(content: str, max_chunk_size: int = DEFAULT_CHUNK_SIZE) -> list[dict]:
        ...
        for part in parts:
            if len(part) <= max_chunk_size:
                ...
                for sub in ChunkingStrategies.fixed(
                    part, max_chunk_size, ChunkingStrategies.DEFAULT_OVERLAP
                ):
...
class ChunkStore:
    ...
    def store_chunks(
        self, memory_id: str, chunks_data: list[dict], embeddings: list[list[float]] | None = None
    ) -> list[Chunk]:
        stored: list[Chunk] = []
        ids: list[str] = []
