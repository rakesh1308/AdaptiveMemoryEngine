...
_REST_TOOL_FNS: dict[str, Callable[..., str]] = {}
...
def _register_tools(mcp, engine: MemoryEngine) -> None:
    ...
    def store_memory(
        key: str, content: str, tags: list[str] | None = None, auto_tag: bool = False
    ) -> str:
    ...
    @mcp.tool(
        name="smart_search", description="Alias for search (kept for backward compatibility)."
    )
    ...
    @mcp.tool(
        name="list_memories", description="List memories, optionally filtered by text or tag."
    )
    ...
    @mcp.tool(
        name="query_graph",
        description="Query the knowledge graph: related concepts or path-finding.",
    )
    def query_graph(
        concept: str | None = None, depth: int = 1, find_path_to: str | None = None
    ) -> str:
        return json.dumps(
    ...
    @mcp.tool(
        name="ask", description="Ask a question using retrieved memories as context (RAG-style)."
    )
    def ask(question: str, context_limit: int = 3) -> str:
        return engine.ask(question, context_limit=int(context_limit))

    @mcp.tool(
        name="summarize", description="Summarize memories matching a query, or specific keys."
    )
    def summarize(
        query: str | None = None,
    ...
    def get_provider_info() -> str:
        ...
        intel_cfg = (
            engine.intelligence_provider.get_config() if engine.intelligence_provider else None
        )
        return json.dumps(
            {
    ...
    def import_memories(items: list[dict]) -> str:
        ...
        existing_content = {
            (m.get("content") or "").strip().lower() for m in engine.list_memories(limit=100_000)
        }
    ...
    def import_chat_export(
        file_path: str, platform: str = "auto", tag_prefix: str = "imported"
    ) -> str:
        return _mcp_import_chat(
            engine, file_path=file_path, platform=platform, tag_prefix=tag_prefix
        )
    ...
    def summarize_memory(key: str) -> str:
        return _mcp_summarize_memory(engine, key=key)


# ---- REST tool registry (used by /api/tools/{name}, HTTP-Action plugin) ----


def _build_rest_tools(engine: MemoryEngine) -> dict[str, Callable[..., str]]:
...
def _mcp_export(engine, *, format="json", include_versions=False):
    ...
    if format == "csv":
        ...
        w = csv.writer(buf)
        w.writerow(
            ["id", "content", "tags", "importance", "createdAt", "updatedAt", "version", "source"]
        )
        for m in memories:
            w.writerow(
                [
...
def _mcp_import(engine, *, items=None, items_json=None):
    ...
    for item in items:
        ...
        if content.strip().lower() in existing_content:
            ...
            continue
        engine.store_memory(
            mid, content, tags=item.get("tags") or [], auto_tag=False, source="import"
        )
        existing_ids.add(mid)
        existing_content.add(content.strip().lower())
        added += 1
...
def _mcp_import_chat(engine, *, file_path, platform="auto", tag_prefix="imported"):
    cfg = Config.load()
    import_root = Path(cfg.import_root) if cfg.import_root else engine.data_dir / "imports"
    ...
    try:
        ...
        return f"Import path must be inside: {root}"
    if not p.exists():
        return f"File not found inside import root: {p.name}"
    if p.stat().st_size > 50_000_000:
        return "Import file exceeds the 50 MB limit."
    try:
        raw = json.loads(p.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError):
        log.exception("Failed to read chat export %s", p)
        return "Failed to read a valid UTF-8 JSON export."
    if not isinstance(raw, list):
        return "Chat export must be a JSON array."
    if platform == "auto":
    ...
    for item in items:
        if item["id"] in existing_ids or item["content"].strip().lower() in existing_content:
            ...
            continue
        engine.store_memory(
            item["id"], item["content"], tags=item["tags"], auto_tag=False, source="chat-import"
        )
        existing_ids.add(item["id"])
        existing_content.add(item["content"].strip().lower())
        added += 1
...
def _mcp_summarize_memory(engine, *, key):
    ...
    if engine.intelligence_provider:
        try:
            ...
            log.exception("AI memory summary failed for %s", key)
...
def _build_mcp_server(engine: MemoryEngine):
    ...
    try:
        from mcp.server.fastmcp import FastMCP as _Server  # type: ignore[attr-defined]
    except ImportError:
...
def build_http_app(engine: MemoryEngine):
    ...
    cfg = Config.load()
    mcp = _build_mcp_server(engine)
    # Reverse-proxy host validation is opt-in. An explicitly configured list
    # enables strict SDK DNS-rebinding protection; an absent list permits PaaS
    # proxy hostnames whose public value is not known at build time.
    from mcp.server.streamable_http import TransportSecuritySettings

    enforce_allowed_hosts = bool(cfg.allowed_hosts)
    security = TransportSecuritySettings(
        enable_dns_rebinding_protection=enforce_allowed_hosts,
    ...
    try:
        ...
        try:
            from mcp.server.fastmcp import FastMCP as _FastMCP  # type: ignore[attr-defined]

            mcp2 = _FastMCP(name=SERVER_NAME, transport_security=security)
    ...
    async def root(_request):
        s = await anyio.to_thread.run_sync(engine.get_stats)
        return JSONResponse(
            {
                "name": SERVER_NAME,
    ...
    async def health(_request):
        try:
            s = await anyio.to_thread.run_sync(engine.get_stats)
            backend_health = getattr(engine.sqlite, "health", None)
            storage = (
                await anyio.to_thread.run_sync(backend_health)
                if backend_health
                else {"ok": True, "backend": "sqlite"}
            )
            return JSONResponse(
                {
                    "status": "ok",
            ...
            return JSONResponse(
                {"status": "unhealthy", "engineReady": False}, status_code=503
            )
    ...
    async def call_tool(request: _Req) -> _JR:
        ...
        if content_length:
            try:
                if int(content_length) > 2_000_000:
                    return _JR({"error": "request_too_large"}, status_code=413)
            except ValueError:
                return _JR({"error": "invalid_content_length"}, status_code=400)
        try:
            payload = await request.json()
        except Exception:
            return _JR({"error": "invalid_json"}, status_code=400)
        if not isinstance(payload, dict):
            return _JR({"error": "invalid_payload"}, status_code=400)
        ...
        try:
            if not isinstance(arguments, dict):
                return _JR({"error": "arguments_must_be_an_object"}, status_code=400)
            result = await anyio.to_thread.run_sync(partial(tool_fn, **arguments))
            return _JR({"result": result})
        except TypeError as e:
            return _JR({"error": f"bad_arguments: {e}"}, status_code=400)
        except Exception:  # noqa: BLE001
            log.exception("REST tool %s failed", name)
            return _JR({"error": "internal_server_error"}, status_code=500)

    base_app.router.routes.append(Route("/api/tools/{name}", endpoint=call_tool, methods=["POST"]))

    # CORS — browser clients must be explicitly allowlisted in production.
    ...
    if not cfg.auth_token:
        log.warning("HTTP transport has no AUTH_TOKEN; bind only to a trusted private network")
    if not enforce_allowed_hosts:
        log.warning(
            "HTTP transport has no ALLOWED_HOSTS; reverse-proxy Host validation is disabled"
        )
    ...
    class BearerAuthMiddleware:
        ...
        async def __call__(self, scope, receive, send) -> None:
            path = scope.get("path", "")
            protected = path == "/mcp" or path.startswith("/mcp/") or path.startswith("/api/tools/")
            if self.token and protected and scope.get("method") != "OPTIONS":
                headers = {k.lower(): v for k, v in scope.get("headers", [])}
                supplied = headers.get(b"authorization", b"").decode("latin-1")
                expected = f"Bearer {self.token}"
                if not hmac.compare_digest(supplied, expected):
                    response = _JR({"error": "unauthorized"}, status_code=401)
                    await response(scope, receive, send)
                    return
            await self.app(scope, receive, send)

    protected_app = BearerAuthMiddleware(base_app, cfg.auth_token)
    cors = CORSMiddleware(
        app=protected_app,
...
def serve() -> None:
    ...
    engine = MemoryEngine(
        ...
        database_pool_min=cfg.database_pool_min,
        database_pool_max=cfg.database_pool_max,
    )
    engine.initialize()
    log.info("Engine ready: %d memories", engine.get_stats()["total"])
    try:
