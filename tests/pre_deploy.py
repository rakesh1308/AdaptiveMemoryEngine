...
Runs against an isolated temporary directory and validates the full pipeline:
...
TEST_DATA_CONTEXT = tempfile.TemporaryDirectory(prefix="ame-predeploy-")
TEST_DATA = TEST_DATA_CONTEXT.name
...
# ----------------------------------------------------------------------------
try:
    backend = SQLiteBackend(TEST_DATA)
    backend.initialize()
    backend.insert({"id": "predeploy-python", "content": "Python powers distributed systems.", "tags": ["python"], "importance": 50})
    backend.save_embedding("predeploy-python", [0.0] * 1536)
...
# ----------------------------------------------------------------------------
try:
    kg = KnowledgeGraph(TEST_DATA)
    kg.build_from_memory("predeploy-python", "Python powers Distributed Systems.", ["python"])
    kg.save()
...
try:
    ...
    eng = MemoryEngine(embedding_provider=emb, intelligence_provider=intel, data_dir=TEST_DATA)
    eng.initialize()
...
try:
    ...
    eng = MemoryEngine(embedding_provider=emb, data_dir=TEST_DATA)
    eng.initialize()
...
try:
    proc = subprocess.Popen(
        [sys.executable, "-m", "adaptive_memory_engine.server"],
        stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        cwd=str(ROOT), env={**os.environ, "DATA_DIR": TEST_DATA, "TRANSPORT": "stdio", "PYTHONIOENCODING": "utf-8"},
    )
    init_req = json.dumps({
